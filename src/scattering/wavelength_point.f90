...
contains
    ...
    subroutine initialize_from_eps_and_mu(this, lambda, number_of_layers, eps, mu)
        ...
        call log_message('Created wavelength point', DETAIL, FILE_DESCRIPTOR(DETAIL))
        call log_wavelength_point(this, DETAIL, FILE_DESCRIPTOR(DETAIL))
    ...
    !  general logger for the wavelength point with level and file descriptor
    subroutine log_wavelength_point(point, level, fd)
        ...
        write(fd, *) 'Wavelength point: lambda = ', 2q0 * PI / point%k, ' k = ', point%k
        write(fd, *) 'eps = ', point%eps
        write(fd,*) 'mu = ', point%mu
        write(fd,*) 'ri = ', sqrt(point%eps * point%mu)
        call log_new_line(level, fd)

    end subroutine log_wavelength_point
